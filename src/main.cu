...
int main(int argc, char * argv[])
{
    ...
    // GPU Stats from CUDA Samples...
    FLOG("Flash Attention Inference start with %u CPU processes on the %u-th GPU: %s",
         FLAGS_cpu_procs,
         FLAGS_gpu_rank,
         dev_prop.name);
    ...
    FLOG("CUDA driver version / runtime version: %d.%d / %d.%d",
         driver_version / 1000,
         (driver_version % 100) / 10,
         runtime_version / 1000,
         (runtime_version % 100) / 10);
    FLOG("CUDA capability major/minor version number: %d.%d",
         dev_prop.major,
         dev_prop.minor);
    FLOG("%d multiprocessors, %d CUDA cores/MP: %d CUDA cores",
    ...
    FLOG("L2 cache size: %.0f KBytes (%d Bytes)",
         static_cast<double>(dev_prop.l2CacheSize) / 1024,
    ...
    FLOG("Max dimension size of a thread block (x,y,z): (%d, %d, %d)",
         dev_prop.maxThreadsDim[0],
         dev_prop.maxThreadsDim[1],
         dev_prop.maxThreadsDim[2]);
    FLOG("Max dimension size of a grid size (x,y,z): (%d, %d, %d)",
    ...
    FLOG("FMHA: "
         "Softmax (Q (%u x %u x %u x %u) * "
         "K^T (%u x %u x %u x %u)) * "
         "V (%u x %u x %u x %u) = "
         "O (%u x %u x %u x %u)",
         FLAGS_b, FLAGS_sq, FLAGS_hq, FLAGS_d,
         FLAGS_b, FLAGS_sk, FLAGS_hk, FLAGS_d,
         FLAGS_b, FLAGS_sk, FLAGS_hk, FLAGS_d,
         FLAGS_b, FLAGS_sq, FLAGS_hq, FLAGS_d);

    FLOG("Profiling: "
         "is causal: %d, num splits: %d, stream: %p, is alibi: %d, is hybrid: %d, prefill fraction: %u, "
    ...
    Tester tester(FLAGS_b, FLAGS_sq, FLAGS_sk, FLAGS_hq, FLAGS_hk, FLAGS_d,
