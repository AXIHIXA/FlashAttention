...
public:
            ...
            m_max(max)
    {
        ...
        FAI_CHECK_CUDART_ERROR(cudaMalloc(&m_dev_ptr, m_elem_num * sizeof(T)));
        FAI_CHECK(m_dev_ptr);
        ...
        // Use fp32 here and cast to T, to support host-managed fp16 tensors.
        std::uniform_real_distribution<float> d(m_min, m_max);

        for (size_t i = 0; i < m_elem_num; ++i)
        {
            m_host_ptr[i] = static_cast<T>(d(e));
        }
    ...
    void checkValue(Tensor<T> * base)
    {
        ...
        for (size_t i = 0; i < m_elem_num; ++i)
        {
            diff = static_cast<double>(
                    std::abs(
                            static_cast<float>(m_host_ptr[i]) -
                            static_cast<float>(base->getHostPtr()[i])
                    )
            );
